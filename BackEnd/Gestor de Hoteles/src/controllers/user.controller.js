    ...
    try{
        ...
        if(usuario){
            return res.status(400).send({
                message: `Un usuario ya posee el corrreo: ${email}`,
                ok: false,
                usuario: usuario,
            });
        }
        ...
        res.status(200).send({
            message: `Usuario creado correctamente: ${usuario.name}`,
            ok: true,
            usuario,
            token: token,
        });
        ...
        res.status(200).send({
            message: `Usuario creado correctamente: ${name}`,
            ok: true,
            usuario: usuario,
        });
...
const readUser = async(req, res) =>{
    ...
    return res.status(200).send({
        message: 'Solo el Administrador autorizado puede listar los usuarios'
        });
    }
};
...
const updateUser = async(req, res) =>{
    try{
        ...
        if(usercomplete){
            ...
            res
            .status(404).send({
                message: 'Este usuario no existe, verificar parámetros.'
            })
        };
