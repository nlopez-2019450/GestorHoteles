    ...
    if(req.user.rol === 'ADMINAPP'){
     const {namehotel, address, admin} = req.body;
         try{
             ...
             res.status(500).json({
                 ok: false,
                 message: 'No se puedo crear el hotel',
                 error: error,
             })
         }
    }else{
        return res.status(500).send({
             message: 'Solo El Administrador autorizado puede agregar hoteles'
         })
    }
}
...
const updateHotel = async(req, res)=>{
   if(req.user.rol === 'ADMINAPP'){
    ...
    return res.status(500).send({
            message: 'Solo el administrador autorizado puede editar el hotel'
        })
   }
} 
...
const deleteHotel = async(req, res) =>{
    if(req.user.rol === 'ADMINAPP'){
        ...
        return res.status(500).send({
            message: 'Solo el administrador autorizado puede eliminar el hotel'
        })
    }
};
